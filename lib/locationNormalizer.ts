...
import { getCountryFromCity, CITY_TO_COUNTRY } from './countryFlags';
...
export function normalizeCountry(country: string | null | undefined): string {
  ...
  const allCountryVariations = [
    ...
    'espana', 'deutschland', 'osterreich', 'nederland', 'belgique',
...
export function normalizeCity(city: string | null | undefined): string {
  ...
  if (/^(praha|dublin|paris|berlin|amsterdam)\s*\d+/i.test(normalized)) {
    const baseCity = normalized.match(/^(praha|dublin|paris|berlin|amsterdam)/i)?.[1];
    if (baseCity) {
      const baseLower = baseCity.toLowerCase();
      if (CITY_NORMALIZATION_MAP[baseLower] || baseLower === 'praha') return 'Prague';
  ...
  if (countryNames.includes(normalized) || COUNTRY_NORMALIZATION_MAP[lowerCity]) {
