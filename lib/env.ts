...
const schema = z.object({
	...
	POLAR_SUCCESS_URL: z
		.string()
		.url()
		.default("https://getjobping.com/success?checkout_id={CHECKOUT_ID}"),
...
if (isBuildTime) {
	...
	if (openaiKey) {
		console.log(
			`🔍 Build: OPENAI_API_KEY is set (${openaiKey.length} chars, starts with: ${openaiKey.substring(0, 5)}...)`,
		);
		if (!openaiKey.startsWith("sk-")) {
			console.warn(
				`⚠️  Build: OPENAI_API_KEY doesn't start with 'sk-', but continuing build...`,
			);
		}
	} else {
		console.warn(
			"⚠️  Build: OPENAI_API_KEY not found, using placeholder for build validation",
		);
	}
	...
	const buildEnv = {
		...
		OPENAI_API_KEY: openaiKey?.startsWith("sk-")
			? openaiKey
		...
		POLAR_ACCESS_TOKEN:
			process.env.POLAR_ACCESS_TOKEN || "build-dummy-token-for-validation-only",
	...
	if (!buildResult.success) {
		...
		console.warn(
			"⚠️  Some environment variables have validation issues during build:",
		);
		buildResult.error.issues.forEach((err) => {
			...
			console.warn(
				`    Value: ${value ? `'${String(value).substring(0, 20)}...'` : "undefined"}`,
			);
		});
	...
	if (!parseResult.success) {
		...
		parseResult.error.issues.forEach((err) => {
			...
			console.error(
				`    Current value: ${value ? `'${value.substring(0, 20)}...' (${value.length} chars)` : "undefined/empty"}`,
			);
		});
		...
		if (isVercel) {
			...
			if (hasCriticalErrors) {
				...
				console.warn(
					"⚠️  Non-critical environment variable issues detected on Vercel",
				);
				...
				const fallbackEnv = {
					...process.env,
					RESEND_API_KEY:
						process.env.RESEND_API_KEY || "re_deployment_placeholder",
					INTERNAL_API_HMAC_SECRET:
						process.env.INTERNAL_API_HMAC_SECRET ||
						"deployment-placeholder-secret-32-chars-minimum",
					SYSTEM_API_KEY:
						process.env.SYSTEM_API_KEY || "deployment-placeholder-key",
					OPENAI_API_KEY:
						process.env.OPENAI_API_KEY || "sk-deployment-placeholder",
				};
				const fallbackResult = schema.safeParse(fallbackEnv);
				if (fallbackResult.success) {
					ENV = fallbackResult.data;
					console.warn(
						"⚠️  Using fallback values for missing environment variables",
					);
				} else {
					// Even with fallbacks, if it still fails, log but try to continue
					console.error(
						"❌ Fallback validation also failed, but continuing anyway to prevent deployment failure",
					);
					console.error(
						"⚠️  This may cause runtime errors - please fix environment variables",
					);
