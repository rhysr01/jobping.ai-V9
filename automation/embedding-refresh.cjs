...
function runEmbeddingRefresh(trigger = 'manual') {
  if (isRunning) {
    console.log(`⚠️  Embedding refresh already running, skipping triggered by ${trigger}`);
  ...
  return new Promise((resolve) => {
    ...
    child.on('close', (code) => {
      isRunning = false;
      if (code === 0) {
        console.log(`✅ Embedding refresh complete at ${new Date().toISOString()}`);
      } else {
        console.error(`❌ Embedding refresh failed with exit code ${code} at ${new Date().toISOString()}`);
      }
      resolve();
    });
...
(async () => {
  ...
  console.log(`   • Command: ${refreshCommand}`);
  console.log('   • Use EMBEDDING_REFRESH_CRON / EMBEDDING_REFRESH_COMMAND env vars to customise');
