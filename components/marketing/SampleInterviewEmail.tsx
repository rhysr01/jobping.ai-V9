...
import { getCareerPathLabel } from "@/Utils/matching/categoryMapper";
...
function _getFallbackJobs(careerPath: string): Job[] {
...
function _formatDescription(description: string): string {
...
function _getMatchScore(index: number): number {
	// Vary match scores for realism
	const scores = [87, 94, 89, 85, 88];
	return scores[index] || 85;
}

function _isHotMatch(score: number): boolean {
	return score >= 92; // Raised from 90 to 92 for rarer hot matches
}

function getUniqueMatchReason(
	job: Job,
	_score: number,
	index: number,
	careerPath: string = "finance",
): string {
	...
	if (careerPath === "finance") {
		...
		return (
			reasons[index] ||
			`Matches your preferences: ${city}, Finance career path, visa sponsorship available, and entry-level friendly.`
		);
	} else if (careerPath === "tech") {
		const reasons = [
		...
		return (
			reasons[index] ||
			`Matches your preferences: ${city}, Tech career path, visa sponsorship available, and entry-level friendly.`
		);
	} else {
		// Strategy (default)
		const reasons = [
		...
		return (
			reasons[index] ||
			`Matches your preferences: ${city}, Strategy career path, visa sponsorship available, and entry-level friendly.`
		);
	}
}
		...
		async function fetchJobs() {
			try {
				const response = await fetch(
					`/api/sample-jobs?day=${day}&tier=premium`,
				);
		...
		return (
			<div className="mx-auto w-full max-w-[360px] px-2 text-[14px] leading-[1.5] text-zinc-100 font-sans">
				<div className="mb-6 rounded-t-lg bg-gradient-to-br from-brand-600 via-brand-500 to-brand-700 p-6 text-center">
					<div className="text-[32px] font-bold text-white tracking-tight mb-2">
						JobPing
					</div>
					<div className="text-[11px] text-white/95 uppercase tracking-wider font-medium">
		...
		<div className="mx-auto w-full max-w-[360px] px-2 text-[14px] leading-[1.5] text-zinc-100 font-sans">
			...
			<div className="rounded-lg border border-zinc-800/50 bg-white/[0.03] p-5 shadow-[0_4px_6px_rgba(0,0,0,0.1)] backdrop-blur-sm">
				...
				<div className="mb-6 p-4 rounded-lg bg-gradient-to-r from-brand-500/20 to-purple-600/20 border-2 border-brand-500/40">
					<p className="text-[14px] text-brand-200 font-bold mb-1 uppercase tracking-wide">
						💎 Premium Feature Preview
					</p>
					<p className="text-[13px] text-zinc-300 leading-relaxed">
						Example: Your Monday email (Premium only). Free users see matches
						instantly, no emails.
					</p>
				</div>

				<h1 className="text-[22px] font-bold text-white mb-4 leading-tight">
					Your {displayJobs.length} new{" "}
					{displayJobs.length === 1 ? "match" : "matches"} are ready ✨
				</h1>
				<p className="text-[15px] text-zinc-300 leading-relaxed mb-4">
					Here's what our matcher surfaced for you today. Every role below
					cleared the filters you set - location, career path, visa, and
					early-career fit.
				</p>

				{/* Profile Section - Use real data when available, fallback for showcase */}
				<div className="mb-4 p-4 rounded-xl bg-brand-500/10 border border-brand-500/30">
					<div className="text-[13px] text-brand-200 font-bold mb-2 uppercase tracking-wide">
						📋 Your Profile
					</div>
					<div className="text-[13px] text-zinc-300 leading-relaxed space-y-1">
						{userProfile?.careerPath && (
							<div>
								<strong>Career Path:</strong>{" "}
								{getCareerPathLabel(userProfile.careerPath)}
							</div>
						)}
						{userProfile?.cities && userProfile.cities.length > 0 ? (
							<div>
								<strong>Cities:</strong>{" "}
								{userProfile.cities.slice(0, 5).join(", ")}
								{userProfile.cities.length > 5
									? ` + ${userProfile.cities.length - 5} more`
									: ""}
							</div>
						) : (
							<div>
								<strong>Cities:</strong> London, Amsterdam, Berlin
							</div>
						)}
						<div>
							<strong>Visa:</strong> Visa Sponsorship Required
						</div>
						<div>
							<strong>Level:</strong> Graduate Programs
						</div>
						<div>
							<strong>Work Style:</strong> Hybrid
						</div>
					</div>
				</div>

				<p className="text-[15px] text-zinc-400 leading-relaxed mb-4">
					Review the highlights, tap through to apply, and let us know if
					anything feels off - your feedback powers the next batch.
				</p>
				...
				{displayJobs.map((job, index) => {
					...
					const uniqueMatchReason =
						job.matchReason ||
						getUniqueMatchReason(
							job,
							uniqueScore,
							index,
							userProfile?.careerPath || careerPath,
						);
					const _experienceText = job.isGraduate
						? "Entry-level graduate role"
						: job.isInternship
							? "Internship (fits your profile)"
							: "Entry-level (fits your profile)";
					const hasJobHash = !!job.jobHash;
					const isFeedbackSubmitted = job.jobHash
						? feedbackSubmitted.has(job.jobHash)
						: false;
					...
					return (
						...
						>
							{/* Match Score + Company (top row like actual email) */}
							<div className="mb-4 flex justify-between items-start">
								<span
									aria-label={`${uniqueScore} percent job match score${hot ? ", hot match" : ""}`}
									className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 font-semibold text-white border ${
										uniqueScore >= 92
											? "bg-emerald-500/20 text-emerald-400 border-emerald-500/30 text-[12px]"
											: uniqueScore >= 85
												? "bg-purple-500/20 text-purple-400 border-purple-500/30 text-[12px]"
												: "bg-zinc-500/20 text-zinc-200 border-zinc-500/40 text-[12px]"
									}`}
								>
									{hot ? `🔥 ` : ""}
									{uniqueScore}% Match
								...
								<div className="text-right">
									<div className="text-[14px] text-zinc-200 font-semibold">
										{job.company}
									</div>
								</div>
							</div>
							...
							{/* Job Title */}
							<div className="font-bold text-[20px] text-white mb-2 leading-tight">
								{job.title}
							</div>

							{/* Location */}
							<div className="text-[14px] text-zinc-300 mb-4">
								📍 {job.location}
							</div>

							{/* AI Match Reason - Match actual email styling */}
							{uniqueMatchReason && (
								<div className="mb-4 p-3 rounded-lg bg-purple-500/15 border-l-[3px] border-purple-500/50">
									<div className="text-[12px] text-purple-400 font-semibold mb-1 uppercase tracking-wide">
										🤖 Why This Matches
									</div>
				...
				<div className="mt-6 pt-5 border-t border-purple-500/12 text-center">
					<div className="text-purple-400 font-semibold text-[14px] mb-2">
						JobPing
					</div>
					<div className="text-[11px] text-zinc-500">
						This is a sample email preview showing real matches from a real
						user. Sign up to get your own personalized matches.
					</div>
				</div>
			</div>
		</div>
