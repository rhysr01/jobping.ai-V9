...
export default function HeroBackgroundAura({ offset = 0, enableMotion = true }: HeroBackgroundAuraProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute top-[-20%] left-1/2 -translate-x-1/2 h-[820px] w-[820px] md:h-[900px] md:w-[900px] blur-md-hero opacity-80 rounded-full bg-[radial-gradient(circle_at_center,theme(colors.brand.500/0.32),theme(colors.zinc.950/0.2)_60%)] animate-[hero-blob_9s_ease-in-out_infinite_alternate]"
        style={{
      ...
      <div
        className="absolute top-[10%] left-1/2 -translate-x-1/2 h-[580px] w-[580px] md:h-[640px] md:w-[640px] blur-md-hero opacity-90 rounded-full bg-[radial-gradient(circle_at_center,_theme(colors.brand.500/0.45),_transparent_55%)] mix-blend-screen"
        style={{
          willChange: enableMotion ? 'transform, opacity' : 'auto',
          transform: enableMotion 
            ? `translate(calc(-42% + ${pos.x}px), calc(-28% + ${offset}px + ${pos.y}px)) rotate(-6deg)`
            : 'translate(-42%, -28%) rotate(-6deg)',
        }}
        onMouseMove={enableMotion ? (e) => {
      ...
      <div className="absolute inset-x-0 top-[-25%] h-[300px] bg-gradient-to-b from-brand-500/15 to-transparent blur-md-hero" />
      
      {/* Side ambient glow */}
      <div className="absolute left-[-12%] top-[12%] h-[520px] w-[520px] bg-[radial-gradient(circle_at_center,_theme(colors.brand.300/0.28),_transparent_70%)] blur-lg-hero opacity-80" />
