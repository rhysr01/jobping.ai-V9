...
export function HeroGeometric() {
	...
	const [_statsStale, setStatsStale] = useState(false);
		...
		const fetchStats = async () => {
			try {
				...
				if (data) {
					...
					setActiveJobsTarget(
						hasFreshStats && activeJobs > 0 ? activeJobs : 12748,
					);
					setInternshipsTarget(
						hasFreshStats && internships > 0 ? internships : 4997,
					);
					setGraduatesTarget(hasFreshStats && graduates > 0 ? graduates : 3953);
					setTotalUsersTarget(
						hasFreshStats && totalUsers > 0 ? totalUsers : 3400,
					);
			...
			<div className="relative z-10 container mx-auto px-4 md:px-6">
				<div className="max-w-4xl mx-auto text-center">
					...
					<motion.div
						...
						initial="hidden"
						animate="visible"
						className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/[0.08] border border-white/[0.08] mb-8 md:mb-12"
					>
						...
						<div className="flex flex-wrap items-center justify-center gap-3 text-xs text-zinc-400">
							...
							{(hasInternships || hasGraduates) && (
								<div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-1.5">
									{hasInternships && (
										<span>{`${formatNumber(displayInternships)} internships`}</span>
									)}
									{hasInternships && hasGraduates && (
										<span className="text-zinc-500">•</span>
									)}
									{hasGraduates && (
										<span>{`${formatNumber(displayGraduates)} graduate programmes`}</span>
									)}
								</div>
							)}
							{hasTotalUsers && (
