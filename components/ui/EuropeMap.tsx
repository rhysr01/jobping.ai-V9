...
export default function EuropeMap({ 
  ...
  return (
    <div 
      className={`relative w-full h-full min-h-[380px] sm:min-h-[450px] md:min-h-[500px] lg:min-h-[550px] rounded-2xl border border-brand-500/20 overflow-hidden shadow-glow-strong ${className}`}
      role="application"
          ...
          {cityEntries.map(([city, coords]) => {
            ...
            return (
              <g key={city} aria-label={`${city}, ${coords.country}`}>
                ...
                <motion.text
                  x={coords.x}
                  y={coords.y - (selected ? 24 : hovered || focused ? 22 : 20)}
                  textAnchor="middle"
