...
export function EarlyAccessBanner() {
	...
	useEffect(() => {
		apiCallJson<{ success: boolean; data?: { count: number } }>(
			"/api/stats/signups",
		)
			.then((data) => {
				...
				<div className="relative z-10 flex flex-col items-center gap-3 md:gap-4">
					...
					<div className="flex items-center justify-center gap-2.5">
						...
						>
							...
							{/* Subtle glow effect */}
							<div className="absolute inset-0 text-2xl md:text-3xl blur-md opacity-50 text-brand-400">
								⚡
							</div>
						</motion.div>
						<p className="text-sm md:text-base font-semibold text-white">
							Early Access: First 300 users get{" "}
							<span className="text-brand-200">Premium free</span> for 1 month
						</p>
					</div>
					...
					<div className="flex flex-wrap items-center justify-center gap-3 text-xs md:text-sm">
						...
						<span className="text-zinc-400">
							<span className="font-medium text-brand-200">{remaining}</span>{" "}
							spots left
						</span>
					</div>
