    ...
    <section className="pt-10 pb-10 md:pt-12 md:pb-12 bg-black scroll-snap-section relative">
      <div className="container-page relative z-10">
        <motion.div
          ...
          viewport={{ once: true }}
          transition={{ duration: 0.5 }}
          className="mx-auto max-w-3xl text-left sm:text-center mb-6"
        >
          <motion.span
            ...
            className="inline-flex items-center gap-2 rounded-full border border-brand-500/40 bg-brand-500/10 px-3 py-1 text-[10px] font-medium tracking-[0.16em] uppercase text-brand-200 shadow-lg shadow-brand-500/10"
          >
            <BrandIcons.Target className="h-3 w-3 text-brand-300" />
            Active Countries
          </motion.span>
          ...
          <p className="text-base text-zinc-300 md:text-lg leading-relaxed">
            We operate in these countries
          </p>
        </motion.div>

        <div 
          className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-4 max-w-5xl mx-auto"
          role="list"
          ...
          {countries.map((countryData, index) => {
            ...
            return (
              <motion.div
                ...
                className="group relative flex flex-col items-center gap-2 p-4 rounded-xl bg-gradient-to-br from-white/[0.08] to-white/[0.04] border border-white/10 backdrop-blur-xl shadow-feature transition-all duration-300 hover:border-white/20 hover:shadow-hover overflow-hidden"
                role="listitem"
                aria-label={`${countryData.country}: ${countryData.cities.length > 0 ? countryData.cities.join(', ') : countryData.country}`}
              >
                {/* Subtle gradient overlay on hover */}
                <div className="absolute inset-0 bg-gradient-to-br from-brand-500/0 via-brand-500/0 to-purple-500/0 group-hover:from-brand-500/10 group-hover:via-brand-500/5 group-hover:to-purple-500/10 transition-all duration-300 rounded-xl" />
                
                {/* Flag - smaller */}
                <div className="relative z-10">
                  <span className="text-2xl md:text-3xl leading-none drop-shadow-lg" role="img" aria-label={countryData.country}>
                    {countryData.flag}
                  </span>
                </div>
                
                {/* Cities as badges */}
                <div className="relative z-10 flex flex-wrap items-center justify-center gap-1.5 w-full min-h-[1.5rem]">
                  {countryData.cities.length > 0 ? (
                    countryData.cities.map((city, cityIndex) => (
                      <motion.span
                        ...
                        className="inline-flex items-center px-2 py-0.5 rounded-md bg-white/5 border border-white/10 text-[10px] font-medium text-zinc-300 backdrop-blur-sm transition-all duration-200 group-hover:bg-white/8 group-hover:border-white/15"
                      >
                        {city}
                      </motion.span>
                    ))
                  ) : (
                    <span className="text-[10px] font-medium text-zinc-400">{countryData.country}</span>
                  )}
                </div>
                
                {/* Subtle shine effect on hover - using CSS classes instead of inline styles */}
                <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-xl shine-effect" />
              </motion.div>
            );
          })}
        </div>
