...
function classifyEarlyCareer(job) {
    ...
    const hasEarlyCareerKeyword = earlyCareerKeywords.some(keyword => text.includes(keyword));
    ...
    const hasEarlyCareerExperience = experiencePatterns.some(pattern => pattern.test(text));
    ...
    const hasEducationRequirement = educationPatterns.some(pattern => pattern.test(text));
