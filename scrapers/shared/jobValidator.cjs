...
const JOB_BOARDS = [
  'reed', 'indeed', 'linkedin', 'adzuna', 'totaljobs', 
  'monster', 'ziprecruiter', 'jobspy', 'google', 'glassdoor',
  'careerjet', 'jooble', 'arbeitnow', 'reed.co.uk', 'indeed.com',
  'linkedin.com', 'adzuna.co.uk', 'totaljobs.com'
];
...
function validateJob(job) {
  ...
  const isJobBoard = JOB_BOARDS.some(board => 
    companyLower.includes(board) || 
    companyNameLower.includes(board)
  );
  
  if (isJobBoard) {
    errors.push(`Job board detected as company: ${job.company || job.company_name}`);
  }
