...
async function runDataQualityAudit() {
	...
	if (conflictingFlags && conflictingFlags > 0) {
		results.push({
			section: "WARNING - Data Inconsistency",
			issue:
				"Jobs with both is_internship=true AND is_graduate=true (mutually exclusive)",
	...
	if (remoteJobs && remoteJobs > 0) {
		results.push({
			...
			issue:
				"Remote jobs in database (scraper should filter these at line 368)",
			count: remoteJobs,
	...
	const _criticalIndexes = [
		{
			name: "idx_jobs_city",
			reason: "CRITICAL for city-based filtering in matching queries",
		},
		{
			name: "idx_jobs_categories_gin",
			reason: "CRITICAL for category/career path filtering",
		},
		{
			name: "idx_jobs_is_active_status",
			reason: "CRITICAL for active job filtering",
		},
	];
	...
	results.push({
		...
		issue:
			"Check indexes manually: idx_jobs_city, idx_jobs_categories_gin, idx_jobs_is_active_status",
		count: 0,
		percentage: 0,
		severity: "info",
	...
	if (sourceStats) {
		const sourceMap = new Map<
			string,
			{
				...
				missingWorkEnv: number;
			}
		>();

		sourceStats.forEach((job) => {
			const stats = sourceMap.get(job.source) || {
				total: 0,
				missingCity: 0,
				missingCategories: 0,
				missingWorkEnv: 0,
		...
		for (const [source, stats] of sourceMap.entries()) {
			const cityCoverage = (
				((stats.total - stats.missingCity) / stats.total) *
				100
			).toFixed(1);
			const categoryCoverage = (
				((stats.total - stats.missingCategories) / stats.total) *
				100
			).toFixed(1);
			const workEnvCoverage = (
				((stats.total - stats.missingWorkEnv) / stats.total) *
				100
			).toFixed(1);
			...
			console.log(
				`  City coverage: ${cityCoverage}% (${stats.missingCity} missing)`,
			);
			console.log(
				`  Category coverage: ${categoryCoverage}% (${stats.missingCategories} missing)`,
			);
			console.log(
				`  Work env coverage: ${workEnvCoverage}% (${stats.missingWorkEnv} missing)`,
			);
