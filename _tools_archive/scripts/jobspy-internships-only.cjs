	...
	for (let i = 0; i < unique.length; i += BATCH_SIZE) {
		...
		try {
			...
			if (result.error) {
				...
				console.log(
					`✅ Saved ${slice.length} internship jobs (batch ${Math.floor(i / BATCH_SIZE) + 1})`,
				);
...
async function main() {
	...
	const OTHER_CITIES = Object.keys(INTERNSHIP_TERMS).filter(
		(c) => !PRIORITY_CITIES.includes(c),
	);
	const cities = [...PRIORITY_CITIES, ...OTHER_CITIES];

	const RESULTS_WANTED = parseInt(
		process.env.JOBSPY_INTERNSHIP_RESULTS || "20",
		10,
	); // More results for internships
	const PRIORITY_RESULTS_WANTED = parseInt(
		process.env.JOBSPY_PRIORITY_INTERNSHIP_RESULTS || "35",
		10,
	); // Even more for priority cities
...
`,
			...
			if (py.status !== 0) {
				console.error(
					`❌ Failed for "${term}" in ${city}:`,
					py.stderr?.trim() || `status ${py.status}`,
				);
				continue;
			}
