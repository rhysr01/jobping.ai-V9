...
console.log(
	`✅ Loaded ${Object.keys(result.parsed || {}).length} variables from .env.local`,
);
...
async function generateAllEmbeddings() {
	...
	console.log(
		"🔍 Debug: Sample of loaded env vars (first 20):",
		allEnvKeys.slice(0, 20),
	);
	console.log("🔍 Debug: Total env vars loaded:", allEnvKeys.length);

	// Debug: Check what OpenAI-related env vars exist
	const openaiVars = Object.keys(process.env).filter(
		(key) =>
	...
	if (!process.env.OPENAI_API_KEY || process.env.OPENAI_API_KEY !== openaiKey) {
		...
		console.log(
			`✅ Cleaned and set OPENAI_API_KEY (${openaiKey.length} chars, starts with ${openaiKey.substring(0, 7)}...)`,
		);
	}
	...
	console.log(
		`   OpenAI API Key: ${process.env.OPENAI_API_KEY ? `Set (${process.env.OPENAI_API_KEY.substring(0, 7)}...)` : "Missing"}\n`,
	);
	...
	while (true) {
		...
		try {
			...
			console.log(
				`✓ Processed ${jobs.length} jobs, estimated cost: $${batchCost.toFixed(6)}`,
			);
			console.log(
				`  Total processed: ${totalProcessed}, Total cost: $${totalCost.toFixed(6)}`,
			);
	...
	console.log(
		`Jobs without embeddings: ${coverage.total - coverage.withEmbeddings}`,
	);
}
