...
function formatEnvFile(
	...
	const categories: Record<string, string[]> = {
		...
		"Email (Resend)": ["RESEND_API_KEY", "EMAIL_DOMAIN"],
		"Payments (Polar)": [
			"POLAR_ACCESS_TOKEN",
			"POLAR_WEBHOOK_SECRET",
			"POLAR_SUCCESS_URL",
		],
		"Caching (Redis)": ["REDIS_URL", "CACHE_TTL_MS"],
		"Security & Authentication": [
			"INTERNAL_API_HMAC_SECRET",
			"ADMIN_API_KEY",
			"ADMIN_BASIC_USER",
	...
	const remaining = Array.from(vars.keys()).filter(
		(k) => validVars.has(k) && !Object.values(categories).flat().includes(k),
	);
...
function main() {
	...
	for (const [key, _value] of envVars.entries()) {
	...
	console.log(
		"   2. If it looks good, replace .env.local with the cleaned version:",
	);
