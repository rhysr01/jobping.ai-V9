...
async function enrichShortDescriptions() {
	...
	while (true) {
		...
		const _updates: Array<{ id: number; description: string }> = [];
		...
		for (const job of shortJobs) {
			...
			console.log(
				`  Job ${job.id}: "${job.title}" at ${job.company} - ${job.description?.length || 0} chars`,
			);
	...
	console.log(
		"Note: Automatic expansion disabled. Review short descriptions manually.",
	);
}
