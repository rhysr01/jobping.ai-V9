...
function MatchEvidencePageContent() {
  ...
  const fetchEvidence = async () => {
    try {
      const response = await apiCall(`/api/matches/evidence?jobHash=${encodeURIComponent(jobHash)}&email=${encodeURIComponent(email)}&token=${encodeURIComponent(token)}`);
      
      if (!response.ok) {
        const data = await response.json().catch(() => ({ error: 'Failed to load match evidence' }));
      ...
      setError(err instanceof Error ? err.message : 'Failed to load match evidence');
      setLoading(false);
    }
  };
  ...
  const scoreColor = evidence.match.match_score >= 80 
    ? 'text-green-400' 
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <div className="bg-zinc-900 rounded-lg p-6 mb-6 border border-zinc-800">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-gray-400">{evidence.job.company} • {evidence.job.location}</p>
            </div>
          ...
          {!evidence.job.is_active && (
            <div className="bg-yellow-900/20 border border-yellow-500/30 rounded-lg p-4 mb-4">
              <p className="text-yellow-400 text-sm">
                ⚠️ This role may have been filled. The evidence below is still valuable for understanding your match profile.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="bg-zinc-900 rounded-lg p-6 mb-6 border border-zinc-800">
          ...
          <div className="prose prose-invert max-w-none">
            <p className="text-gray-300 leading-relaxed whitespace-pre-line">
              {evidence.match.match_reason || 'AI-matched based on your preferences'}
            </p>
          </div>
          
          {/* Highlighted Skills & Keywords */}
          {evidence.highlightedMatches && evidence.highlightedMatches.length > 0 && (
            <div className="mt-6 pt-6 border-t border-zinc-800">
              <h4 className="text-lg font-semibold mb-3 text-purple-400">What Triggered This Match</h4>
              <div className="flex flex-wrap gap-2">
                {evidence.highlightedMatches.map((match: string, idx: number) => (
                  <span
                    key={idx}
          ...
          <div className="space-y-3">
            <div>
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="text-gray-400">{evidence.match.match_score}%</span>
              </div>
        ...
        <div className="flex gap-4">
          <Button
            ...
            className="flex-1"
          >
            {applying ? 'Opening...' : evidence.job.is_active ? 'Apply on Source →' : 'Job May Be Filled'}
          </Button>
        </div>

        {/* Footer */}
        <div className="mt-8 text-center text-sm text-gray-500">
          <p>This match was generated on {new Date(evidence.match.matched_at).toLocaleDateString()}</p>
          <p className="mt-2">
            <a href="/preferences" className="text-purple-400 hover:text-purple-300">
