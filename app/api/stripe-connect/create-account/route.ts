  ...
  try {
    ...
    if (!isStripeConfigured()) {
      return NextResponse.json(
        { error: 'Stripe Connect is not configured. Please set STRIPE_SECRET_KEY.' },
        { status: 503 }
      );
    }
    ...
    if (existingAccount) {
      apiLogger.info('User already has Connect account', { userId, accountId: existingAccount.account_id });
    ...
    if (dbError) {
      apiLogger.error('Failed to store Connect account in database', dbError as Error, { 
        accountId: account.id,
        userId 
      });
    ...
    return NextResponse.json({
      ...
      message: 'Account created successfully. Call /api/stripe-connect/create-account-link to start onboarding.',
    });
