...
export async function GET(req: NextRequest) {
  try {
    ...
    if (ENV.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY) {
      ...
      checks.publishableKeyFormat = ENV.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY.startsWith('pk_');
      
      if (!checks.publishableKeyFormat) {
        checks.errors.push('NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY must start with "pk_"');
      }
    } else {
    ...
    if (ENV.STRIPE_CONNECT_WEBHOOK_SECRET) {
      ...
      if (!ENV.STRIPE_CONNECT_WEBHOOK_SECRET.startsWith('whsec_')) {
        checks.errors.push('STRIPE_CONNECT_WEBHOOK_SECRET should start with "whsec_"');
      }
    } else {
      checks.errors.push('STRIPE_CONNECT_WEBHOOK_SECRET is not set (optional for local dev)');
    }
    ...
    if (ENV.STRIPE_WEBHOOK_SECRET) {
      ...
      checks.errors.push('STRIPE_WEBHOOK_SECRET is not set (optional for local dev)');
    }
    ...
    if (checks.stripeConfigured) {
      try {
        ...
        checks.errors.push(`Failed to initialize Stripe client: ${error.message}`);
      }
    }
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        webhooksConfigured: checks.connectWebhookSecretPresent && checks.billingWebhookSecretPresent,
      },
      message: allGood 
