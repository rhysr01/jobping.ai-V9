...
async function recordImplicitSignal(signalData: ImplicitSignalData) {
  ...
  const { error } = await supabase.from("implicit_signals").insert({
...
async function _getUserBehaviorInsights(userEmail: string) {
  try {
    ...
    const dwellTimes = signals
      .filter((s) => s.signal_type === "dwell" && s.value)
      .map((s) => s.value || 0);
    ...
    const clickedJobs = signals
      .filter((s) => s.signal_type === "click" && s.jobs)
      .map((s) => s.jobs);
    const jobTitles = clickedJobs
      .flat()
    ...
    return {
      ...
      daysActive: new Set(
        signals.map((s) => new Date(s.created_at).toDateString()),
      ).size,
    };
  } catch (error) {
