...
export async function GET(req: NextRequest) {
  try {
    ...
    if (usersError) {
      apiLogger.error('Failed to fetch users without matches', usersError as Error);
    ...
    const uniqueUsersWithMatches = new Set(recentMatches?.map(m => m.user_email) || []);
    ...
    const zeroMatchRate = totalActiveUsers > 0 
    ...
    return NextResponse.json({
      ...
      status: zeroMatchRate < 1 ? 'healthy' : zeroMatchRate < 5 ? 'warning' : 'critical',
      recommendations: zeroMatchRate >= 5 ? [
