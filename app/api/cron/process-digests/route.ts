...
export async function POST(request: NextRequest) {
  try {
    ...
    if (fetchError) {
      ...
      return NextResponse.json(
        { error: 'Failed to fetch pending digests', details: fetchError.message },
        { status: 500 }
      );
    }
    ...
    for (const digest of readyDigests) {
      try {
        ...
        const { data: user, error: userError } = await supabase
          .from('users')
          .select('email, full_name, subscription_tier, active, delivery_paused, last_email_sent, email_count')
        ...
        if (user.last_email_sent) {
          ...
          if (lastSent >= twentyFourHoursAgo) {
            ...
            const newScheduledFor = new Date(lastSent.getTime() + 24 * 60 * 60 * 1000);
            await supabase
              .from('pending_digests')
        ...
        const activeJobs = (jobs || [])
          .map(job => {
            const matchData = jobHashes.find(j => j.job_hash === job.job_hash);
            return {
              ...job,
        ...
        // Get user preferences for distribution
        const { data: userPrefs } = await supabase
          .from('users')
          .select('target_cities, work_environment')
          .eq('email', digest.user_email)
          .single();
        ...
        if (userPrefs?.work_environment) {
          if (Array.isArray(userPrefs.work_environment)) {
            ...
            targetWorkEnvironments = userPrefs.work_environment.split(',').map(env => env.trim()).filter(Boolean);
          }
        }

        // Apply job distribution for diversity
        const distributedJobs = distributeJobsWithDiversity(activeJobs as any[], {
        ...
        // Mark as sent to prevent retry loops (or implement retry logic later)
          ...
          if (updateError) {
            ...
            apiLogger.warn('Failed to mark digest as sent', updateError as Error, { digestId: digest.id });
          }
      }
    }
