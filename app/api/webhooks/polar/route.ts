...
async function handleOrderPaid(payload: PolarWebhookPayload) {
  ...
  if (existingUser?.subscription_active === true) {
    apiLogger.info(`User ${customerEmail} already has active subscription (idempotency)`);
  ...
  if (updateError) {
    apiLogger.error('Failed to activate subscription', updateError as Error, { email: customerEmail });
    return { success: false, error: updateError.message };
  }

  apiLogger.info(`✅ Activated premium subscription for ${customerEmail}`, { orderId: order.id });
...
async function handleCheckoutEvent(payload: PolarWebhookPayload) {
  ...
  apiLogger.debug(`Checkout event: ${payload.type}`, { checkoutId: checkout.id });
...
async function handleSubscriptionEvent(payload: PolarWebhookPayload) {
  ...
  if (!customerEmail) {
    ...
    if (subscription.customer_id) {
      apiLogger.debug(`Subscription event but no email, will sync via customer_id`, { 
        subscriptionId: subscription.id,
        customerId: subscription.customer_id 
      });
      // We'll handle this by checking subscription status directly
    } else {
      apiLogger.warn('Subscription event but no email or customer_id found', { subscriptionId: subscription.id });
  ...
  if (customerEmail) {
    ...
    if (!user) {
      apiLogger.warn(`User not found for subscription event`, { email: customerEmail });
      return { success: false, error: 'User not found' };
    }

    if (user.subscription_active === isActive) {
      apiLogger.info(`User ${customerEmail} subscription status unchanged (${status})`);
    ...
    if (updateError) {
      apiLogger.error('Failed to update subscription', updateError as Error, { email: customerEmail });
      return { success: false, error: updateError.message };
    }

    apiLogger.info(`✅ Updated subscription for ${customerEmail}: ${status} (active: ${isActive})`);
...
async function handleSubscriptionCanceled(payload: PolarWebhookPayload) {
  ...
  if (!user) {
    apiLogger.warn(`User not found for subscription cancellation`, { email: customerEmail });
    return { success: false, error: 'User not found' };
  }

  if (user.subscription_active === false) {
    apiLogger.info(`User ${customerEmail} subscription already inactive (idempotency)`);
  ...
  if (updateError) {
    apiLogger.error('Failed to deactivate subscription', updateError as Error, { email: customerEmail });
    return { success: false, error: updateError.message };
  }
...
async function handleCustomerEvent(payload: PolarWebhookPayload) {
  ...
  apiLogger.debug(`Customer event: ${payload.type}`, { customerId: customer.id, email: customer.email });
...
export const POST = Webhooks({
  ...
  onPayload: async (payload: PolarWebhookPayload): Promise<void> => {
    ...
    apiLogger.info(`Polar webhook received`, { eventType, payloadId: payload.data?.id });
    ...
    try {
      ...
      switch (eventType) {
        ...
        case 'order.updated':
        case 'order.refunded':
          apiLogger.debug(`Order event: ${eventType}`, { orderId: payload.data.id });
          result = { success: true, message: `Order event logged: ${eventType}` };
          break;
        
        // Other events - log but don't process
        default:
          apiLogger.debug(`Unhandled Polar webhook event type: ${eventType}`, { payloadId: payload.data?.id });
