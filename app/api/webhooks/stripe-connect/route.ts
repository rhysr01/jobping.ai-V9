...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!webhookSecret) {
      apiLogger.error('STRIPE_CONNECT_WEBHOOK_SECRET not configured', new Error('Missing webhook secret'));
      return NextResponse.json(
    ...
    switch (event.type) {
      case 'account.updated': {
        ...
        if (updateError) {
          apiLogger.error('Failed to update account in database', updateError as Error, {
      ...
      case 'account.application.deauthorized': {
        ...
        if (deauthError) {
          apiLogger.error('Failed to mark account as deauthorized', deauthError as Error);
        }
