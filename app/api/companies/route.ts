...
import { getCompanyLogo, getAllCompanyLogos } from '@/lib/companyLogos';
 ...
 */
function getGuaranteedFallbackCompanies(): Array<{ name: string; logoPath: string }> {
  try {
    const allLogos = getAllCompanyLogos();
    if (allLogos.length === 0) {
      console.error('[Companies API] CRITICAL: getAllCompanyLogos() returned empty array!');
...
export const GET = asyncHandler(async (req: NextRequest) => {
  ...
  try {
    ...
    if (error) {
      ...
      if (dbCompaniesWithLogos.length > 0) {
        ...
        console.log(`[Companies API] No DB matches found, using guaranteed fallback`);
        companiesWithLogos = getGuaranteedFallbackCompanies();
      }
    }
  } catch (error) {
  ...
  if (!companiesWithLogos || companiesWithLogos.length === 0) {
    console.error('[Companies API] CRITICAL: companiesWithLogos is empty! Using emergency fallback.');
    companiesWithLogos = getGuaranteedFallbackCompanies();
  }
