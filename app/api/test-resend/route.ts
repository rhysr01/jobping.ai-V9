...
export const GET = async (req: NextRequest) => {
	...
	try {
		...
		try {
			...
			apiLogger.debug("API Key valid", {
				domainCount: Array.isArray(domains.data) ? domains.data.length : 0,
			});

			// Check if getjobping.com is verified
			const domainsList = Array.isArray(domains.data) ? domains.data : [];
			const getjobpingDomain = domainsList.find(
				(d: any) => d.name === "getjobping.com",
			);
			if (getjobpingDomain) {
		...
		try {
			apiLogger.debug("Testing email send", {
				config: EMAIL_CONFIG,
      ...
      `,
			...
			if (error) {
				...
				apiLogger.error("Email send failed", error as Error, {
					...
					requestId:
						error?.response?.headers?.get?.("x-resend-request-id") ?? "n/a",
					from: EMAIL_CONFIG.from,
				});
			} else {
			...
			const _status = error?.status ?? "unknown";
			const rid =
				error?.response?.headers?.get?.("x-resend-request-id") ?? "n/a";
			const _body = await error?.response?.json?.().catch(() => error?.message);
		...
		const envTest = {
			...
			allResendVars: Object.keys(process.env).filter((k) =>
				k.includes("RESEND"),
			),
			...
			diagnostics: {
				...
				looksValid:
					apiKey.startsWith("re_") && apiKey.length > 20 && !/\s/.test(apiKey),
			},
		};
		...
		return NextResponse.json({
			...
			summary: {
				...
				overallStatus:
					apiKeyTest.success && emailTest.success ? "SUCCESS" : "FAILED",
			},
		});
	} catch (error: any) {
		...
		if (error.message?.includes("Missing Resend API key")) {
			errorHint =
				"RESEND_API_KEY is not set in environment. Check .env.local (local) or Vercel environment variables (production).";
		} else if (error.message?.includes("Invalid Resend API key format")) {
			errorHint =
				'RESEND_API_KEY must start with "re_". Check your API key format.';
		} else if (error.message?.includes("timeout")) {
			errorHint =
				"Resend API call timed out. This might indicate a network issue or Resend API being slow.";
		}
