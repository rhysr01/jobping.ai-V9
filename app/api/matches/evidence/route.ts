...
export async function GET(request: NextRequest) {
	try {
		...
		const { data: job, error: jobError } = await supabase
			.from("jobs")
			.select(
				"id, title, company, location, description, job_url, job_hash, is_active, categories",
			)
			.eq("job_hash", jobHash)
			.single();
...
function extractHighlightedMatches(
	...
	if (jobLocation) {
		const locationMatch = matchReason.match(
			new RegExp(`\\b${jobLocation.split(",")[0]}\\b`, "i"),
		);
	...
	if (jobCategories && jobCategories.length > 0) {
		...
		if (
			matchReason.toLowerCase().includes(category.toLowerCase()) &&
			!seen.has(formattedCategory)
		) {
