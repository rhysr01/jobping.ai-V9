...
async function queryWithTimeout<T>(
  ...
  try {
    ...
    if (error instanceof Error && error.message.includes("timeout")) {
      apiLogger.error(`Database query timeout: ${operation}`, error as Error, {
        timeoutMs,
        operation,
      });
      return {
  ...
  try {
    ...
    if (userResult.error) {
      if (userResult.error.code === "TIMEOUT") {
        apiLogger.error(
          "User query timeout",
          new Error(userResult.error.message),
          { email },
        );
        return NextResponse.json(
    ...
    if (matchesResult.error) {
      if (matchesResult.error.code === "TIMEOUT") {
        apiLogger.error(
          "Matches query timeout",
          new Error(matchesResult.error.message),
          { email },
        );
        return NextResponse.json(
          { error: "Request timed out. Please try again." },
      ...
      apiLogger.error("Failed to fetch matches", matchesResult.error as Error, {
        email,
      });
    ...
    if (
      ...
      try {
        const { getTargetCompaniesFromHistory } =
          await import("@/Utils/matching/guaranteed/historical-alerts");
        ...
        if (userPrefs) {
          ...
          const { data: customScanData } = await supabase
            .from("custom_scans")
            .select("id, estimated_completion, created_at")
            .eq("user_email", email)
            .eq("status", "pending")
        ...
        apiLogger.warn("Failed to fetch target companies/custom scan", {
    ...
    if (jobHashes.length === 0) {
      apiLogger.warn("Matches found but no job_hashes", {
        email,
        matchesCount: matchesData.length,
      });
      return NextResponse.json({ jobs: [] });
    ...
    const jobsQuery = supabase
      .from("jobs")
      .select(
        "id, title, company, location, city, country, description, job_url, work_environment, categories, job_hash, is_active, status",
      )
    ...
    if (jobsResult.error) {
      if (jobsResult.error.code === "TIMEOUT") {
        apiLogger.error(
          "Jobs query timeout",
          new Error(jobsResult.error.message),
          { email, jobHashesCount: jobHashes.length },
        );
        return NextResponse.json(
          { error: "Request timed out. Please try again." },
          { status: 504 },
      ...
      apiLogger.error("Failed to fetch jobs", jobsResult.error as Error, {
        email,
        jobHashesCount: jobHashes.length,
      });
