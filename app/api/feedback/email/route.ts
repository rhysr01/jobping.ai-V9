...
export async function POST(request: NextRequest) {
  try {
    ...
    const action = searchParams.get('action') as 'positive' | 'negative' | 'neutral';
    const score = parseInt(searchParams.get('score') || '0');
    const email = searchParams.get('email');

    if (!action || !email || isNaN(score)) {
    ...
    const { error: logError } = await supabase
      ...
      .insert({
        ...
        error_message: action === 'negative' ? 'User provided negative feedback' : null,
        ai_cost_usd: 0,
      ...
      <body>
        <div class="container">
          ...
          <div class="message">
            ...
            ${action === 'positive' ? 'We\'re glad you found the matches useful!' : 
              action === 'negative' ? 'We\'ll work on better matches for you.' : 
