...
import { CheckCircle, XCircle, Loader2, ExternalLink, Package } from 'lucide-react';
...
function DashboardContent() {
  ...
  const loadAccountStatus = async () => {
    try {
      ...
      // Get account details
      const accountRes = await fetch(`/api/stripe-connect/get-account?accountId=${accountId}`);
      if (accountRes.ok) {
        const accountData = await accountRes.json();
        setAccountStatus({
          accountId,
          onboardingComplete: accountData.account.chargesEnabled && accountData.account.payoutsEnabled,
              ...
              <div className="rounded-xl bg-white/[0.06] border border-white/10 backdrop-blur-xl px-6 py-8">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <code className="text-sm text-brand-300">{accountStatus.accountId}</code>
                  </div>
                ...
                {accountStatus.requirements.currentlyDue.length > 0 && (
                  <div className="mt-4 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                    <p className="text-yellow-200 text-sm font-medium mb-2">Requirements to Complete:</p>
