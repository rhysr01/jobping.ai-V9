...
describe('Verify Email API Route – Comprehensive', () => {
  ...
  it('verifies email with valid token', async () => {
    ...
    const request = new NextRequest('http://localhost/api/verify-email?email=user@example.com&token=token123');
    ...
    expect(verifyVerificationToken).toHaveBeenCalledWith('user@example.com', 'token123');
    expect(markUserVerified).toHaveBeenCalledWith('user@example.com');
  });

  it('rejects requests missing the email parameter', async () => {
    const request = new NextRequest('http://localhost/api/verify-email?token=token123');
    const response = await GET(request);
    expect(response.status).toBe(400);
  });

  it('rejects requests missing the token parameter', async () => {
    const request = new NextRequest('http://localhost/api/verify-email?email=user@example.com');
    const response = await GET(request);
    expect(response.status).toBe(400);
  });

  it('propagates verification errors', async () => {
    verifyVerificationToken.mockResolvedValue({ valid: false, reason: 'Token expired' });
    const request = new NextRequest('http://localhost/api/verify-email?email=user@example.com&token=expired');
