...
import { POST, GET } from '@/app/api/track-engagement/route';
...
describe('Track Engagement API Route', () => {
  ...
  describe('POST /api/track-engagement', () => {
    it('should track email opened', async () => {
      ...
      expect(updateUserEngagement).toHaveBeenCalledWith('user@example.com', 'email_opened');
    });
    ...
    it('should track email clicked', async () => {
      ...
      expect(response.status).toBe(200);
      expect(updateUserEngagement).toHaveBeenCalledWith('user@example.com', 'email_clicked');
    });
  ...
  describe('GET /api/track-engagement', () => {
    ...
    it('should track email opened and return pixel', async () => {
      mockRequest.url = 'https://example.com/api/track-engagement?email=user@example.com&type=email_opened';
    ...
    it('should track email clicked and redirect', async () => {
      mockRequest.url = 'https://example.com/api/track-engagement?email=user@example.com&type=email_clicked&url=https%3A%2F%2Fexample.com%2Fjob';
    ...
    it('should require email and type parameters', async () => {
      mockRequest.url = 'https://example.com/api/track-engagement?email=user@example.com';

      await expect(GET(mockRequest)).rejects.toThrow();
    });

    it('should validate type parameter', async () => {
      mockRequest.url = 'https://example.com/api/track-engagement?email=user@example.com&type=invalid';
