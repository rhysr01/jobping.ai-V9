...
describe("GET /api/dashboard - Contract Tests", () => {
	...
	describe("Database Metrics", () => {
		beforeEach(async () => {
			...
			await supabase.from("users").insert([
				...
				{ email: "test2@example.com", subscription_tier: "premium", active: true },
			]);

			await supabase.from("jobs").insert([
				{ job_hash: "job1", title: "Test Job 1", company: "Test Co", is_active: true, status: "active" },
				{ job_hash: "job2", title: "Test Job 2", company: "Test Co 2", is_active: true, status: "active" },
			]);

			await supabase.from("matches").insert([
				{ user_email: "test1@example.com", job_hash: "job1", match_score: 0.8, match_reason: "Good match" },
				{ user_email: "test2@example.com", job_hash: "job2", match_score: 0.9, match_reason: "Excellent match" },
			]);
		});
	...
	describe("Error Handling", () => {
		it("should handle partial failures gracefully", async () => {
			...
			jest.mocked(require("@/lib/monitoring").performanceMonitor).getMetricStats.mockImplementationOnce(() => {
