...
describe("GET /api/matches/free - Contract Tests", () => {
  ...
  describe("Error Handling", () => {
    ...
    it("should handle database connection errors", async () => {
      ...
      const originalGetDatabaseClient = jest.requireActual(
        "@/Utils/databasePool",
      ).getDatabaseClient;
      const mockSupabase = {
        from: jest.fn(() => {
      ...
      jest
        .mocked(require("@/Utils/databasePool").getDatabaseClient)
        .mockReturnValue(originalGetDatabaseClient());
    });
  ...
  describe("Response Format Contract", () => {
    ...
    it("should sort jobs by active status then match score", async () => {
      ...
      expect(data.jobs[0].match_score).toBeGreaterThan(
        data.jobs[1].match_score,
      );
