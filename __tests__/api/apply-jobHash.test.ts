  ...
  describe("Input Validation", () => {
    it("should return 400 for missing email parameter", async () => {
      ...
      const response = await GET(mockRequest, {
        params: Promise.resolve({ jobHash: testJobHash }),
      });
    ...
    it("should return 400 for missing jobHash parameter", async () => {
      ...
      const response = await GET(mockRequest, {
        params: Promise.resolve({ jobHash: "" }),
      });
      expect(response.status).toBe(400);
    ...
    it("should handle invalid token gracefully", async () => {
      ...
      const mockRequest = {
        ...
        nextUrl: new URL(
          `http://localhost/api/apply/${testJobHash}?email=${testUserEmail}&token=invalid`,
        ),
      } as any;
  ...
  describe("Match Verification", () => {
    ...
    it("should return 404 for non-existent match", async () => {
      ...
      const response = await GET(req, {
        params: Promise.resolve({ jobHash: "nonexistent" }),
      });
    ...
    it("should return 400 for job without URL", async () => {
      ...
      const response = await GET(req, {
        params: Promise.resolve({ jobHash: "no-url-job" }),
      });
      expect(response.status).toBe(400);
