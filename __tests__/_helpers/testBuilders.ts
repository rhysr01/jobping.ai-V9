...
export const buildMockPremiumUser = (
	overrides: Partial<UserPreferences> = {},
): UserPreferences => {
...
export const buildMockFreeUser = (
	overrides: Partial<UserPreferences> = {},
): UserPreferences => {
	return buildMockUser({
 ...
 */
export const buildMatchingScenario = (
	options: {
		userCount?: number;
		jobCount?: number;
	...
	if (includeGraduateJobs) {
		...
		jobs.push(
			buildMockGraduateJob({ job_hash: "grad-2", location: "Berlin, Germany" }),
		);
	}
