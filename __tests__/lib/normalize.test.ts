...
describe('normalize', () => {
  ...
  describe('scoreJob', () => {
    ...
    it('should add points for early career keywords', () => {
      const score = scoreJob('Graduate Programme', 'Entry level position', recentDate, 'other');
      expect(score).toBeGreaterThan(40);
    });

    it('should add points for tracked roles', () => {
      const trackedScore = scoreJob('Title', 'Description', recentDate, 'consulting');
      const untrackedScore = scoreJob('Title', 'Description', recentDate, 'other');
      expect(trackedScore).toBeGreaterThan(untrackedScore);
    });

    it('should cap score at 100', () => {
      const score = scoreJob('Graduate Programme', 'Entry level intern trainee rotation', recentDate, 'consulting');
    ...
    it('should handle multiple early career keywords', () => {
      const score = scoreJob('Graduate Intern', 'Trainee rotation leadership', recentDate, 'other');
