...
describe('config', () => {
  ...
  describe('excludeNoise', () => {
    ...
    it('should include senior level terms', () => {
      expect(excludeNoise.some(term => term.toLowerCase().includes('senior'))).toBe(true);
      expect(excludeNoise.some(term => term.toLowerCase().includes('lead'))).toBe(true);
    });
  });
  ...
  describe('targetCities', () => {
    ...
    it('should include all expected cities', () => {
      const expectedCities = [
        'London', 'Madrid', 'Berlin', 'Hamburg', 'Munich',
