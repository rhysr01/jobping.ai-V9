...
import { getResendClient, getSupabaseClient, assertValidFrom, EMAIL_CONFIG } from '@/Utils/email/clients';
...
describe('Email Clients', () => {
  ...
  describe('assertValidFrom', () => {
    it('should pass with valid from address', () => {
      expect(() => assertValidFrom('JobPing <noreply@getjobping.com>')).not.toThrow();
    });
    ...
    it('should throw error for malformed email', () => {
      expect(() => assertValidFrom('JobPing <invalid-email>')).toThrow('Malformed email address');
    });

    it('should throw error for wrong domain', () => {
      expect(() => assertValidFrom('JobPing <noreply@wrongdomain.com>')).toThrow('Invalid sender domain');
    });
  });
