...
jest.mock('@/Utils/url-helpers', () => ({
  getListUnsubscribeHeader: jest.fn(() => '<https://example.com/unsubscribe>, <mailto:unsubscribe@example.com>')
}));
...
describe('Email Deliverability', () => {
  ...
  describe('validateEmailDeliverability', () => {
    it('should validate email deliverability setup', async () => {
      ...
      jest.spyOn(require('dns').promises, 'resolveTxt').mockResolvedValue([
    ...
    it('should detect SPF issues', async () => {
      jest.spyOn(require('dns').promises, 'resolveTxt').mockRejectedValue(new Error('No SPF record'));
    ...
    it('should detect high bounce rate', async () => {
      ...
      expect(result.issues.some(issue => issue.includes('bounce'))).toBe(true);
    });
  });
