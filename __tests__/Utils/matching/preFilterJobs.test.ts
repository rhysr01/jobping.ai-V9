...
describe("preFilterJobs", () => {
	...
	describe("preFilterJobsByUserPreferencesEnhanced", () => {
		it("should filter jobs by location", () => {
			const filtered = preFilterJobsByUserPreferencesEnhanced(
				mockJobs,
				mockUser,
			);
			...
			expect(
				locations.some(
					(loc) => loc.includes("London") || loc.includes("Berlin"),
				),
			).toBe(true);
		});
		...
		it("should filter by target cities", () => {
			...
			const hasLondonJob = filtered.some((job) =>
				job.location.includes("London"),
			);
