...
describe("Batch Matching Processor", () => {
	...
	beforeEach(() => {
		...
		embeddingService.batchGenerateUserEmbeddings = jest
			.fn()
			.mockResolvedValue(new Map());
		embeddingService.storeUserEmbedding = jest
			.fn()
	...
	describe("processBatch", () => {
		...
		it("should fallback to heuristics when embeddings disabled", async () => {
			...
			expect(
				embeddingService.batchGenerateUserEmbeddings,
	...
	describe("groupUsersBySimilarity", () => {
		it("should group users by embedding similarity", async () => {
			...
			const result = await processor.processBatch(users, [], {
				useEmbeddings: true,
			});
		...
		it("should group users by heuristics when embeddings unavailable", async () => {
			...
			const users = [
				{
					email: "user1@example.com",
					preferences: { target_cities: ["London"], career_path: ["strategy"] },
				},
				{
					email: "user2@example.com",
					preferences: { target_cities: ["London"], career_path: ["strategy"] },
