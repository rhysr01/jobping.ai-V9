...
describe('BatchMatchingProcessor', () => {
  ...
  describe('processBatch', () => {
    it('should process users with heuristic grouping', async () => {
      const users = [
        { email: 'user1@example.com', preferences: buildMockUser({ target_cities: ['London'] }) },
        { email: 'user2@example.com', preferences: buildMockUser({ target_cities: ['London'] }) }
      ];
      const jobs = [buildMockJob()];

      (embeddingService.batchGenerateUserEmbeddings as jest.Mock).mockResolvedValue(new Map());

      const results = await processor.processBatch(users, jobs, { useEmbeddings: false });
    ...
    it('should process users with embedding grouping', async () => {
      ...
      (embeddingService.batchGenerateUserEmbeddings as jest.Mock).mockResolvedValue(embeddings);

      const results = await processor.processBatch(users, jobs, { useEmbeddings: true });
    ...
    it('should handle empty job list', async () => {
      const users = [{ email: 'user1@example.com', preferences: buildMockUser() }];
      const results = await processor.processBatch(users, [], { useEmbeddings: false });
      expect(results.size).toBeGreaterThan(0);
    });
    ...
    it('should respect maxBatchSize option', async () => {
      ...
      await processor.processBatch(users, jobs, { maxBatchSize: 5, useEmbeddings: false });
