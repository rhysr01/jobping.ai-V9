...
describe('normalizers', () => {
  describe('toStringArray', () => {
    ...
    it('should filter out non-string items', () => {
      expect(toStringArray(['a', 123, 'b', null, 'c'])).toEqual(['a', 'b', 'c']);
    });
  });
  ...
  describe('isJob', () => {
    ...
    it('should reject object missing job_hash', () => {
      expect(isJob({ title: 'Test', company: 'Test Co', job_url: 'https://example.com' })).toBe(false);
    });

    it('should reject object missing title', () => {
      expect(isJob({ job_hash: 'test', company: 'Test Co', job_url: 'https://example.com' })).toBe(false);
    });
