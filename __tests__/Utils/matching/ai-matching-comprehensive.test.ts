...
import { AIMatchingService, AIMatchingCache } from '@/Utils/matching/ai-matching.service';
...
describe('AI Matching Service - Comprehensive', () => {
  ...
  describe('performEnhancedAIMatching', () => {
    it('should return matches from cache if available', async () => {
      ...
      const matches = await service.performEnhancedAIMatching(jobs, user as any);
  ...
  describe('convertToRobustMatches', () => {
    it('should convert AI matches to robust format', () => {
      ...
      const robustMatches = service.convertToRobustMatches(aiMatches, user as any, jobs);
    ...
    it('should sort matches by score descending', () => {
      ...
      expect(robustMatches[0].match_score).toBeGreaterThanOrEqual(robustMatches[1].match_score);
    });
  });
