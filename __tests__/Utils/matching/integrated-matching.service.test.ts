...
import { IntegratedMatchingService, integratedMatchingService } from '@/Utils/matching/integrated-matching.service';
...
describe('IntegratedMatchingService', () => {
  ...
  describe('processUsersWithBatchOptimization', () => {
    it('should use batch processing for large user groups', async () => {
      ...
      (batchMatchingProcessor.processBatch as jest.Mock).mockResolvedValue(mockBatchResults);

      const results = await service.processUsersWithBatchOptimization(users, jobs, {
        enabled: true,
        minUsersForBatch: 5
      });
  ...
  describe('getSemanticJobs', () => {
    it('should return semantic jobs when available', async () => {
      ...
      (semanticRetrievalService.isSemanticSearchAvailable as jest.Mock).mockResolvedValue(true);
      (semanticRetrievalService.getSemanticCandidates as jest.Mock).mockResolvedValue(mockJobs);

      const result = await service.getSemanticJobs(userPrefs, 200);

      expect(result).toEqual(mockJobs);
      expect(semanticRetrievalService.getSemanticCandidates).toHaveBeenCalledWith(userPrefs, 200);
    });

    it('should return empty array when semantic search unavailable', async () => {
      const userPrefs = buildMockUser();

      (semanticRetrievalService.isSemanticSearchAvailable as jest.Mock).mockResolvedValue(false);

      const result = await service.getSemanticJobs(userPrefs);

      expect(result).toEqual([]);
      expect(semanticRetrievalService.getSemanticCandidates).not.toHaveBeenCalled();
    });
    ...
    it('should handle semantic search errors gracefully', async () => {
      ...
      (semanticRetrievalService.isSemanticSearchAvailable as jest.Mock).mockResolvedValue(true);
      (semanticRetrievalService.getSemanticCandidates as jest.Mock).mockRejectedValue(
        new Error('Semantic search failed')
      );

      await expect(service.getSemanticJobs(userPrefs)).rejects.toThrow('Semantic search failed');
    });
  });

  describe('singleton instance', () => {
    it('should export singleton instance', () => {
      expect(integratedMatchingService).toBeInstanceOf(IntegratedMatchingService);
    });
  });
});
