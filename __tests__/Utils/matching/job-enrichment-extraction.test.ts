...
describe('job-enrichment extraction functions', () => {
  ...
  describe('extractProfessionalExpertise', () => {
    it('should extract software expertise', () => {
      const result = extractProfessionalExpertise('Software Engineer', 'Building software');
      expect(result).toBe('Software Development');
    });

    it('should extract data expertise', () => {
      const result = extractProfessionalExpertise('Data Scientist', 'Working with data');
      expect(result).toBe('Data Science');
    });

    it('should extract product expertise', () => {
      const result = extractProfessionalExpertise('Product Manager', 'Managing products');
      expect(result).toBe('Product Management');
    });

    it('should extract marketing expertise', () => {
      const result = extractProfessionalExpertise('Marketing Manager', 'Marketing campaigns');
      expect(result).toBe('Marketing');
    });

    it('should extract frontend expertise', () => {
      const result = extractProfessionalExpertise('Frontend Developer', 'React and Vue');
      expect(result).toBe('Frontend Development');
    });

    it('should extract backend expertise', () => {
      const result = extractProfessionalExpertise('Backend Developer', 'Node.js and Python');
      expect(result).toBe('Backend Development');
    });

    it('should return General for unknown', () => {
      const result = extractProfessionalExpertise('Unknown Role', 'Random description');
  ...
  describe('extractCareerPath', () => {
    it('should extract tech path', () => {
      const result = extractCareerPath('Software Engineer', 'Building software');
    ...
    it('should extract marketing path', () => {
      const result = extractCareerPath('Marketing Specialist', 'Marketing campaigns');
      expect(result).toBe('marketing');
    });

    it('should extract sales path', () => {
      const result = extractCareerPath('Sales Representative', 'Selling products');
