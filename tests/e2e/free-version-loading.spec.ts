...
test.describe('Free Version - Loading and Performance', () => {
  ...
  test('Free signup completes and redirects to matches', async ({ page }) => {
    ...
    await expect(page.locator('[role="application"]')).toBeVisible({ timeout: 10000 });
    
    // Use partial text matching - buttons have aria-labels like "Prague, Czech Republic. Click to select"
    const cityButton = page.getByRole('button', { name: /Prague|Warsaw/ }).first();
    await expect(cityButton).toBeVisible({ timeout: 10000 });
    await cityButton.click();
    
    // Select career path - use getByRole for better reliability
    const careerPathButton = page.getByRole('button', { name: /Finance & Investment|Tech & Engineering/ }).first();
    ...
    await Promise.race([
      ...
      page.waitForSelector('text=Redirecting', { timeout: 35000 }).then(async () => {
  ...
  test('Matches page loads within timeout and shows content', async ({ page, context }) => {
    ...
    const loadingIndicator = page.locator('text=Finding your perfect matches').first();
    ...
    try {
      ...
      await Promise.race([
        page.waitForSelector('text=Your', { timeout: 35000 }).then(() => { contentLoaded = 'success'; }),
        page.locator('article').first().waitFor({ timeout: 35000 }).then(() => { contentLoaded = 'success'; }),
        page.locator('[role="list"]').first().waitFor({ timeout: 35000 }).then(() => { contentLoaded = 'success'; }),
        // Error states
        page.waitForSelector('text=Failed to load matches', { timeout: 35000 }).then(() => { contentLoaded = 'error'; }),
        page.waitForSelector('text=expired', { timeout: 35000 }).then(() => { contentLoaded = 'error'; }),
        page.waitForSelector('text=Unauthorized', { timeout: 35000 }).then(() => { contentLoaded = 'error'; }),
        // Empty state
        page.waitForSelector('text=No matches found', { timeout: 35000 }).then(() => { contentLoaded = 'empty'; }),
      ]);
    } catch (error) {
    ...
    console.log(`✅ Matches page loaded in ${loadTime}ms (under 30s limit) - state: ${contentLoaded}`);
  });
  ...
  test('Loading state transitions correctly', async ({ page, context }) => {
    ...
    await page.waitForFunction(
      () => {
        return !document.body.textContent?.includes('Finding your perfect matches');
      },
      { timeout: 35000 }
    );
    ...
    const hasContent = await Promise.race([
      ...
      page.locator('text=Your').first().isVisible().then(() => true).catch(() => false),
      page.locator('article').first().isVisible().then(() => true).catch(() => false),
      // Check for error content
      page.locator('text=Failed to load matches').first().isVisible().then(() => true).catch(() => false),
      page.locator('text=No matches found').first().isVisible().then(() => true).catch(() => false),
      // Check for expired message
      page.locator('text=expired').first().isVisible().then(() => true).catch(() => false),
      // Timeout fallback
  ...
  test('Network timeout handling works correctly', async ({ page, context }) => {
    ...
    try {
      await Promise.race([
        // Error messages
        page.waitForSelector('text=Failed to load matches', { timeout: 40000 }).then(() => { pageLoaded = true; }),
        page.waitForSelector('text=timeout', { timeout: 40000 }).then(() => { pageLoaded = true; }),
        page.waitForSelector('text=Request timed out', { timeout: 40000 }).then(() => { pageLoaded = true; }),
        // Success content
        page.waitForSelector('text=Your', { timeout: 40000 }).then(() => { pageLoaded = true; }),
        page.locator('article').first().waitFor({ timeout: 40000 }).then(() => { pageLoaded = true; }),
      ]);
    } catch (error) {
