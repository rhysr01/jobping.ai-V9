...
async function testEndpoint(url: string, options: { method?: string; headers?: Record<string, string>; body?: string } = {}) {
...
async function runTests() {
  ...
  if (matchesResult.responseTime > 35000) {
    console.log('❌ ERROR: Matches API took longer than 35 seconds - TIMEOUT ISSUE!');
  } else if (matchesResult.responseTime > 30000) {
    console.log('⚠️  WARNING: Matches API took longer than 30 seconds - may indicate slow DB');
  }
  
  if (matchesResult.data?.jobs) {
    console.log(`Jobs Returned: ${matchesResult.data.jobs.length}`);
    if (matchesResult.data.jobs.length > 0) {
      console.log(`First Job: ${matchesResult.data.jobs[0].title} at ${matchesResult.data.jobs[0].company}`);
    }
  }
  ...
  const maxTime = Math.max(healthCheck.responseTime, signupResult.responseTime, matchesResult.responseTime);
