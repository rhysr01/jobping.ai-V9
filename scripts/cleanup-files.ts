...
class FileCleanup {
   ...
   */
  private getAllFiles(dir: string, ignoreDirs: Set<string> = new Set(['node_modules', '.git', '.next', 'dist', 'build', '__pycache__', '.cache'])): void {
    try {
  ...
  private getGitTrackedFiles(): void {
    try {
      const output = execSync('git ls-files', { cwd: this.rootDir, encoding: 'utf-8' });
      output.split('\n').forEach(file => {
        if (file.trim()) {
  ...
  private findFileReferences(): void {
    const codeExtensions = ['.ts', '.tsx', '.js', '.jsx', '.cjs', '.mjs', '.json', '.md', '.yml', '.yaml', '.sql'];
      ...
      for (const migrationFile of migrationFiles) {
        ...
        if (dateMatch) {
          ...
          if (migrationDate < sixMonthsAgo) {
            console.log(`   ℹ️  Old migration found: ${migrationFile} (${migrationDate.toISOString().split('T')[0]}) - Keeping for database history`);
          }
        }
      }
    } catch (error) {
  ...
  private createBackup(): void {
    ...
    try {
      ...
      const backupContent = this.filesToRemove
        .map(f => `${f.path}|${f.size}|${f.lastModified.toISOString()}|${f.reason}`)
        .join('\n');
