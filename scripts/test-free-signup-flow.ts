...
async function testSignup() {
  ...
  try {
    ...
    if (!response.ok) {
      console.log(`❌ Signup failed: ${data.error || data.message || 'Unknown error'}`);
      if (data.details) {
    ...
    if (!matchesResponse.ok) {
      console.log(`❌ Failed to fetch matches: ${matchesData.error || matchesData.message || 'Unknown error'}`);
      return;
    }
    ...
    if (matchesData.jobs && matchesData.jobs.length > 0) {
      ...
      matchesData.jobs.forEach((job: any, i: number) => {
        ...
        console.log(`      Match Score: ${job.match_score ? Math.round(job.match_score * 100) : 'N/A'}%`);
      });
    } else {
    ...
    console.log(`✅ Matches API: ${matchesResponse.status === 200 ? 'Success' : 'Failed'}`);
    console.log(`✅ Jobs Returned: ${matchesData.jobs?.length || 0}`);
