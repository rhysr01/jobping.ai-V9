...
// Working EU job population script that actually saves jobs to database
const axios = require('axios');
const { createClient } = require('@supabase/supabase-js');

console.log('🌍 POPULATING EU JOBS TO DATABASE');
console.log('==================================\n');
...
function convertToDatabaseFormat(job) {
  ...
  return {
    ...
    optimized_title: job.title,
    company_name: job.company,
    city: city,
    country: country,
    optimized_profile_url: job.url,
  ...
  for (const job of jobs) {
    try {
      ...
      const { data, error } = await supabase
        .from('jobs')
        .upsert(dbJob, { onConflict: 'job_hash' });
      
      if (!error) {
        savedJobs.push(job);
        console.log(`  ✅ Saved: ${job.title} at ${job.company} (${job.location})`);
      } else {
        console.log(`  ⚠️ Already exists: ${job.title} at ${job.company}`);
      }
    } catch (err) {
...
async function populateEUJobs() {
  try {
    ...
    // 3. JSEARCH - EU locations only
    console.log('📍 POPULATING JSEARCH JOBS (EU Only):');
    try {
    ...
    console.log(`🌍 Cities covered: Madrid, Berlin, Amsterdam, Paris, Dublin, Manchester, Birmingham, Edinburgh, Glasgow`);
    console.log(`✅ Your database now has non-London EU jobs!`);
