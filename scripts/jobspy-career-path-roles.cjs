...
function _extractIndustries(description) {
...
function _extractCompanySize(description, company) {
	...
	for (let i = 0; i < unique.length; i += BATCH_SIZE) {
		...
		try {
			...
			if (result.error) {
				...
				console.log(
					`✅ Saved ${slice.length} jobs (batch ${Math.floor(i / BATCH_SIZE) + 1})`,
				);
	...
	const CAREER_PATHS = [
		...
		},
		{
			...
			roles: [
				...
				"Business Development Intern (Startup)",
...
async function main() {
	...
	console.log(
		`📋 Found ${allRoles.length} unique roles across all career paths\n`,
	);
	...
	const PRIORITY_RESULTS_WANTED = parseInt(
		...
		10,
	); // More results for priority cities
	...
	const MAX_ROLES_PER_CITY = parseInt(
		process.env.MAX_ROLES_PER_CITY || "20",
		10,
	); // Limit roles per city to avoid too many searches
	const PRIORITY_MAX_ROLES = parseInt(
		process.env.PRIORITY_MAX_ROLES || "30",
		10,
	); // More roles for priority cities

	// Limit roles to search (can be overridden via env)
	const rolesToSearch = process.env.ROLES_TO_SEARCH
		? process.env.ROLES_TO_SEARCH.split(",")
				.map((r) => r.trim())
				.filter(Boolean)
		: allRoles.slice(
				0,
				Math.min(allRoles.length, MAX_ROLES_PER_CITY * cities.length),
			);

	console.log(`🌍 Searching ${cities.length} cities`);
	console.log(
		`🔍 Searching ${rolesToSearch.length} roles (${allRoles.length} total available)\n`,
	);
