...
for (const [name, value] of Object.entries(vars)) {
  ...
  if (isSet) {
    ...
    if (name.includes('SERVICE_ROLE')) {
      if (length < 100) {
        console.log(`   ⚠️  WARNING: Too short! Service role keys are typically 200+ chars`);
      } else {
    ...
    if (name.includes('ANON')) {
      if (length < 100) {
        console.log(`   ⚠️  WARNING: Too short! Anon keys are typically 200+ chars`);
      } else {
...
const hasAnon = !!(vars.SUPABASE_ANON_KEY || vars.NEXT_PUBLIC_SUPABASE_ANON_KEY);
...
if (hasServiceRole && vars.SUPABASE_SERVICE_ROLE_KEY && vars.SUPABASE_SERVICE_ROLE_KEY.length < 100) {
  console.log('⚠️  SUPABASE_SERVICE_ROLE_KEY seems too short');
  console.log('   This might actually be the anon key, not the service role key');
...
if (!url) {
  console.log('❌ No Supabase URL found (NEXT_PUBLIC_SUPABASE_URL or SUPABASE_URL)\n');
