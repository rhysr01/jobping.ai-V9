...
async function main() {
	...
	const totalRequests = Number(
		process.env.LIMITER_TOTAL_REQUESTS || DEFAULT_TOTAL_REQUESTS,
	);
	const concurrency = Number(
		process.env.LIMITER_CONCURRENCY || DEFAULT_CONCURRENCY,
	);
	...
	const runner = async () => {
		try {
			...
			failures += 1;
			console.error(
				`Limiter test request failed: ${error instanceof Error ? error.message : String(error)}`,
			);
		}
	};
	...
	if (throttled === 0) {
		console.error(
			"❌ Rate limiter did not engage – expected at least one 429 response.",
		);
		process.exit(1);
	}
	...
	if (failures > totalRequests * 0.1) {
		console.error(
			"❌ Too many unexpected failures while exercising the limiter.",
		);
		process.exit(1);
	}
