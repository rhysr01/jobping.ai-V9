...
async function debugMatches(email: string) {
  ...
  if (matches && matches.length > 0) {
    console.log('   Job hashes:', matches.map(m => m.job_hash));
  }
  ...
  if (matches && matches.length > 0) {
    ...
    console.log('   Active jobs:', jobs?.filter(j => j.is_active && j.status === 'active').length || 0);
    
    // Check which jobs are missing/inactive
    const activeJobHashes = new Set(
      jobs?.filter(j => j.is_active && j.status === 'active').map(j => j.job_hash) || []
    );
    const inactiveHashes = jobHashes.filter(hash => !activeJobHashes.has(hash));
