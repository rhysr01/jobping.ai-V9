...
async function checkStorageUsage() {
	...
	try {
			...
			while (hasMore) {
				...
				files.forEach((file) => {
					if (file.metadata?.size) {
						bucketSize += parseInt(file.metadata.size, 10);
					}
				});
		...
		for (const bucket of buckets) {
			...
			if (oldFilesError) {
				console.error(
					`   ❌ Error checking old files in ${bucket.name}: ${oldFilesError.message}`,
				);
				continue;
			}
			...
			if (oldFiles && oldFiles.length > 0) {
				...
				if (oldFilesList.length > 0) {
					console.log(
						`   📁 ${bucket.name}: ${oldFilesList.length} files older than 30 days`,
					);
