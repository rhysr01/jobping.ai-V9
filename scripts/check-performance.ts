...
try {
	...
	try {
		...
		execSync(
			'npx lighthouse http://localhost:3000 --only-categories=performance --output=json --output-path=./lighthouse-report.json --chrome-flags="--headless"',
			{
				stdio: "inherit",
			},
		);
		...
		console.log(
			"⚠️  Lighthouse not available. Install with: npm install -g lighthouse\n",
		);
	}
} catch (error) {
