...
async function verify() {
  ...
  console.log(`   Service Role Key: ${serviceRoleKey.length} chars, starts with: ${serviceRoleKey.substring(0, 20)}...`);
  if (anonKey) {
    console.log(`   Anon Key: ${anonKey.length} chars, starts with: ${anonKey.substring(0, 20)}...`);
    if (serviceRoleKey === anonKey) {
      console.error('   ❌ CRITICAL: Service role key and anon key are the SAME!');
      console.error('   This is why RLS is blocking - you\'re using the anon key!');
      process.exit(1);
    } else {
