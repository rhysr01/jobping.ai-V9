 ...
 * This provides much more realistic and comprehensive test data
 */
async function fetchRealTestJobs(): Promise<Job[]> {
	try {
		// Try to fetch real jobs from the database first
		const supabase = getDatabaseClient();

		// Get recent, active, early-career jobs from popular cities
		...
		if (!jobs || jobs.length === 0) {
			console.warn("No real jobs found in database, using synthetic data");
			return getFallbackTestJobs();
		}

		console.log(`✅ Using ${jobs.length} real production jobs for testing`);
		return jobs;
	} catch (error) {
		console.warn("Error fetching real jobs:", error);
		return getFallbackTestJobs();
	}
}

/**
 * Fallback synthetic test jobs if database is empty
...
class ProductionMatchingEngineTester {
	...
	private async testFreeUserMatchCount(): Promise<TestResult> {
		...
		// Result should have at least 2 matches (realistic for limited test data)
		const passed = result.matches.length >= 2;
		...
		console.log(
			`   Average Score: ${Math.round(result.matches.reduce((sum, m) => sum + (m.unifiedScore?.overall || 0), 0) / result.matches.length)}%`,
		);
			...
			details: {
				...
				averageScore:
					result.matches.length > 0
						? Math.round(
								result.matches.reduce((sum, m) => sum + (m.unifiedScore?.overall || 0), 0) /
	...
	private async testFallbackChain(): Promise<TestResult> {
		...
		const hasValidResults =
			...
			fallbackResult.every(
				(match) =>
					match.unifiedScore.overall >= 0 &&
					match.unifiedScore.overall <= 100 &&
					match.matchReason &&
					match.matchReason.length > 10,
			);
	...
	private async testPostAIValidation(): Promise<TestResult> {
		...
		const result = {
			matches: fallbackResult.map((match) => ({
				job_hash: match.job.job_hash,
				match_score: match.unifiedScore.overall,
				match_reason: match.matchReason,
				confidence_score: match.unifiedScore.confidence,
			})),
			method: "fallback",
			totalJobsProcessed: PRODUCTION_TEST_JOBS.length,
			prefilterResults: {
...
async function main(): Promise<void> {
	try {
		...
		// Fetch real production jobs for testing instead of synthetic data
		PRODUCTION_TEST_JOBS = await fetchRealTestJobs();
