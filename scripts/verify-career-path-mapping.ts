...
async function verifyMapping() {
  ...
  const formValues = ['tech', 'finance', 'strategy', 'marketing', 'sales', 'operations', 'data', 'product'];
  ...
  if (financeError) {
    ...
    console.log(`📊 Finance jobs (finance-investment + early-career): ${financeJobs || 0}`);
  }
  ...
  if (financeCityError) {
    ...
    console.log(`📊 Finance jobs in Stockholm/Dublin/Paris: ${financeCityJobs || 0}`);
  }
  ...
  if ((techJobs || 0) === 0 && (financeJobs || 0) === 0) {
    ...
    console.log('   Jobs exist but not in the cities used by sample-jobs route.');
  } else {
