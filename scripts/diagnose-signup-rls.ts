...
async function diagnose() {
  ...
  if (!supabaseUrl || !serviceRoleKey) {
    ...
    console.error('   Make sure SUPABASE_SERVICE_ROLE_KEY is set in production.');
    process.exit(1);
  }
  ...
  if (serviceRoleKey.length < 100) {
    console.warn('   ⚠️  Service role key seems too short (should be ~200+ chars)');
  }
  ...
  try {
    ...
    if (policyError) {
      ...
      if (directError) {
        ...
        console.log('   SELECT tablename, policyname, roles, cmd FROM pg_policies');
        console.log('   WHERE schemaname = \'public\' AND tablename = \'users\' AND cmd = \'INSERT\';');
      } else {
    ...
    if (insertError) {
      ...
      if (insertError.code === '42501') {
        ...
        console.error('\n   Solutions:');
        console.error('   1. Apply migration: migrations/fix_signup_rls_service_role.sql');
        console.error('   2. Verify SUPABASE_SERVICE_ROLE_KEY is set in Vercel');
        console.error('   3. Check that the key is the service_role key, not anon key');
      }
    } else {
