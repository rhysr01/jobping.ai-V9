...
async function diagnose() {
  ...
  if (matches && matches.length > 0) {
    ...
    matches.slice(0, 5).forEach((m: any, i: number) => {
      console.log(`     ${i + 1}. Job Hash: ${m.job_hash}, Score: ${m.match_score}`);
    });
    ...
    const { data: matchedJobs, error: jobsError } = await supabase
      .from('jobs')
      .select('job_hash, title, company, city, is_active, status, filtered_reason')
    ...
    if (matchedJobs) {
      const activeJobs = matchedJobs.filter(j => j.is_active && j.status === 'active');
      const inactiveJobs = matchedJobs.filter(j => !j.is_active || j.status !== 'active');
      const filteredJobs = matchedJobs.filter(j => j.filtered_reason !== null);
      ...
      if (inactiveJobs.length > 0) {
        ...
        inactiveJobs.slice(0, 3).forEach((job: any) => {
          ...
          console.log(`       is_active: ${job.is_active}, status: ${job.status}`);
        });
      }
  ...
  if (userCities.length > 0) {
    ...
    console.log(`   2. Active jobs (is_active=true, status='active', filtered_reason IS NULL): ${activeCityJobs || 0}`);
    console.log(`   3. Early-career jobs: ${earlyCareerJobs || 0}`);
    
    // Get sample jobs
    const { data: sampleJobs } = await supabase
      .from('jobs')
      .select('title, company, city, is_active, status, filtered_reason, is_internship, is_graduate')
      .in('city', Array.from(cityVariations))
      .eq('is_active', true)
      .eq('status', 'active')
      .is('filtered_reason', null)
