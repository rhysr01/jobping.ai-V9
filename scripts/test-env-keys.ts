...
console.log(`   NEXT_PUBLIC_SUPABASE_ANON_KEY: ${hasAnonKey ? '✅ Set' : '⚠️  Not set (optional for server-side)'}`);
...
const looksLikeServiceRole = serviceRoleKey!.length > 100 && serviceRoleKey!.startsWith('eyJ');
console.log(`   Looks like service role key: ${looksLikeServiceRole ? '✅' : '❌'}`);
...
if (hasServiceRoleKey && hasAnonKey) {
  ...
  if (keysMatch) {
    ...
    if (servicePrefix === anonPrefix) {
      console.warn('   ⚠️  WARNING: Keys start with the same prefix. Double-check they\'re different!');
    }
  }
} else {
...
async function testServiceRoleKey() {
  ...
  if (readError) {
    ...
    if (readError.code === '42501') {
      ...
      console.error('   This suggests the key might be anon key, not service role key.');
      process.exit(1);
    }
  } else {
  ...
  if (insertError) {
    ...
    if (insertError.code === '42501') {
      ...
      console.error('\n   Possible causes:');
      console.error('   1. The key is actually the anon key (not service role key)');
      console.error('   2. RLS policies are not configured correctly');
      console.error('   3. The key format is incorrect');
      console.error('\n   Solution:');
      console.error('   1. Verify the key is the service_role key from Supabase Dashboard');
      console.error('   2. Check that RLS policies allow service_role to insert');
      console.error('   3. Run: migrations/fix_signup_rls_service_role.sql in Supabase');
      process.exit(1);
    } else {
