...
async function fixMatches() {
	...
	if (existingMatches && existingMatches.length > 0) {
		...
		await supabase.from("matches").delete().eq("user_email", userEmail);
	...
	if (user.career_path) {
		_careerPathCategories = getDatabaseCategoriesForForm(user.career_path);
	}
	...
	console.log(
		`✅ Using ${jobsForMatching.length} jobs (matching engine will filter and rank)`,
	);
	...
	console.log(
		`✅ Matched ${matchResult.matches.length} jobs (method: ${matchResult.method})`,
	);
	...
	const jobsForDistribution =
		highQualityJobs.length >= 5 ? highQualityJobs : matchedJobsRaw;
	...
	// Filter valid jobs
	const validJobs = finalJobs.filter((job: any) => job?.job_hash);
	...
	console.log(
		"   Job hashes:",
		savedMatches.map((m: any) => m.job_hash).join(", "),
	);
