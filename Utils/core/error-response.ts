 ...
 *
 * Migration guide:
 * - Replace `error-response.badRequest(req, msg)` with `throw new ValidationError(msg)`
 * - Replace `error-response.unauthorized(req, msg)` with `throw new UnauthorizedError(msg)`
 * - Replace `error-response.notFound(req, msg)` with `throw new NotFoundError(msg)`
 * - Replace `error-response.internal(req, msg)` with `throw new AppError(msg, 500)`
 ...
 *   if (!data) return error-response.badRequest(req, 'Missing data');
 * }
 *
 * // New:
