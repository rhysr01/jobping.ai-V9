...
export class HealthChecker {
  ...
  private async checkExternalApis(): Promise<ComponentHealth> {
    ...
    try {
      ...
      return {
        status: failures === 0 ? 'healthy' : failures === results.length ? 'unhealthy' : 'degraded',
  ...
  private async checkAdzuna(): Promise<void> {
    ...
    const response = await fetch(`https://api.adzuna.com/v1/api/jobs/gb/search/1?app_id=${process.env.ADZUNA_API_ID}&app_key=${process.env.ADZUNA_API_KEY}&results_per_page=1`);
  ...
  private getResult(promiseResult: PromiseSettledResult<ComponentHealth>): ComponentHealth {
