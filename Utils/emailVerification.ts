...
export function generateVerificationToken(email: string, ttlMinutes: number = 60 * 24): string {
...
export async function verifyVerificationToken(email: string, token: string): Promise<VerificationResult> {
...
export async function sendVerificationEmail(email: string): Promise<void> {
  ...
  const verification = verifySecureToken(normalizedEmail, token, 'email_verification');

  if (!verification.valid || !verification.expiresAt) {
    throw new Error('Failed to generate verification token');
  }

  await persistVerificationToken(normalizedEmail, token, verification.expiresAt);
...
export async function checkEmailVerificationStatus(email: string): Promise<{ verified: boolean }> {
  const supabase = getDatabaseClient();
  const { data, error } = await supabase
    .from('users')
