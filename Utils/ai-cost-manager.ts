...
export class AICostManager {
	...
	constructor() {
		...
		this.dailyLimits = {
			...
			maxCallsPerUser: parseInt(process.env.AI_MAX_CALLS_PER_USER || "5", 10),
			maxCallsPerDay: parseInt(process.env.AI_MAX_CALLS_PER_DAY || "200", 10),
	...
	async canMakeAICall(
		...
		// Daily cost limit
		if (
			this.costMetrics.dailyCost + estimatedCost >
			this.dailyLimits.maxDailyCost
		) {
			return {
				allowed: false,
	...
	async recordAICall(
		...
		this.costMetrics.userCalls[userEmail] =
			(this.costMetrics.userCalls[userEmail] || 0) + 1;
	...
	private async loadDailyMetrics(): Promise<void> {
		...
		if (data) {
			this.costMetrics.dailyCost = data.reduce(
				(sum: number, log: any) => sum + log.cost_usd,
				0,
			);
