...
export class IntegratedMatchingService {
  ...
  ): Promise<Map<string, { matches: JobMatch[]; method: string; processingTime: number }>> {
    const results = new Map<string, { matches: JobMatch[]; method: string; processingTime: number }>();

    // Use batch processing if enabled and user count is sufficient
    if (
      options.enabled &&
      users.length >= (options.minUsersForBatch || 5)
    ) {
  ...
  async getSemanticJobs(
    ...
    const isAvailable = await semanticRetrievalService.isSemanticSearchAvailable();
    
    if (isAvailable) {
      return await semanticRetrievalService.getSemanticCandidates(userPrefs, limit);
    }
