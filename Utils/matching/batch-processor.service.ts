...
export class BatchMatchingProcessor {
  ...
  private async groupByEmbeddingSimilarity(
    ...
    const userEmbeddings =
      await embeddingService.batchGenerateUserEmbeddings(users);
  ...
  private async groupUsersWithDatabase(
    ...
    for (const user of users) {
      ...
      try {
        ...
        console.warn(
          `Database similarity search failed for ${user.email}, using heuristic:`,
          error,
        );
        this.addToHeuristicSegment(user, segments);
      }
  ...
  private addToHeuristicSegment(
    ...
    const segmentKey = `heuristic_${careerPath}_${cities}_${level}`
      .toLowerCase()
      .replace(/[^a-z0-9_+]/g, "");
