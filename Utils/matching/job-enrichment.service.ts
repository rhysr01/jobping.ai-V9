...
export function enrichJobData(job: Job): EnrichedJob {
	...
	return {
		...
		remoteFlexibility: calculateRemoteFlexibility(
			description,
...
function calculateMarketDemand(job: Job): number {
	...
	if (
		job.categories?.includes("ai") ||
		job.categories?.includes("machine-learning")
	)
		demand += 3;
...
function calculateRemoteFlexibility(
	description: string,
...
function calculateGrowthPotential(
	...
	const keywordCount = growthKeywords.filter((keyword) =>
...
export function extractProfessionalExpertise(
	jobTitle: string,
	description: string,
): string {
...
export function extractCareerPath(
	jobTitle: string,
	description: string,
): string {
	const combinedText = `${jobTitle} ${description}`.toLowerCase();
