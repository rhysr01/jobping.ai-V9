 ...
 */
const LOCATION_BASED_PATTERN = /must be based in|must be located in|based in (?:the )?([A-Z][a-z]+)/i;
...
const KNOWN_SPONSOR_COMPANIES = [
  ...
  'spotify', 'stripe', 'salesforce', 'oracle', 'ibm',
  'accenture', 'deloitte', 'pwc', 'ey', 'ernst', 'kpmg', 'mckinsey',
  'boston consulting', 'bain', 'goldman sachs', 'morgan stanley',
  'jpmorgan', 'barclays', 'hsbc', 'deutsche bank',
  'uber', 'airbnb', 'tesla', 'shopify', 'notion', 'vercel',
];

/**
 * Calculate visa sponsorship confidence for a job
 */
export function calculateVisaConfidence(
  job: {
  ...
  return {
    confidence: 'unknown',
    reason: 'No visa sponsorship mentioned. Many startups sponsor visas even if not listed in the job description.',
    keywordsFound: [],
