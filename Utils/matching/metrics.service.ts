...
export function calculateRecallAt50(
  ...
  const relevantInTop50 = top50Jobs.filter((job) =>
    relevantJobHashes.has(job.job_hash),
  ).length;
...
export async function logMatchMetrics(metrics: MatchMetrics): Promise<void> {
  try {
    ...
    const { error } = await supabase.from("match_metrics").insert({
    ...
    console.log(
      `[METRICS] recall@50=${metrics.recallAt50.toFixed(3)}, nDCG@5=${metrics.ndcgAt5.toFixed(3)}, type=${metrics.matchType}`,
    );
  } catch (error) {
...
export async function getMetricsSummary(days: number = 7): Promise<{
  ...
  try {
    ...
    const avgRecallAt50 =
      data.reduce((sum, m) => sum + (m.recall_at_50 || 0), 0) / data.length;
    const avgNDCGAt5 =
      data.reduce((sum, m) => sum + (m.ndcg_at_5 || 0), 0) / data.length;
