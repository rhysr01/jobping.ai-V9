...
import { normalizeCity, normalizeCountry, CITY_NORMALIZATION_MAP, COUNTRY_NORMALIZATION_MAP } from '@/lib/locationNormalizer';
 ...
 */
function normalizeLocationForMatching(location: string | null | undefined): string {
  if (!location) return '';
 ...
 */
function normalizeCountryForMatching(country: string | null | undefined): string {
  if (!country) return '';
 ...
 */
function parseLocationString(location: string): { city: string | null; country: string | null } {
...
export function matchesLocation(
  ...
  if (!job.location && !job.city && !job.country) {
    return { matches: false, matchScore: 0, reason: 'Job has no location data' };
  }

  // Normalize job location data
  const jobCity = job.city ? normalizeCityForMatching(job.city) : null;
  const jobCountry = job.country ? normalizeCountryForMatching(job.country) : null;
  const jobLocation = job.location ? normalizeLocationForMatching(job.location) : null;
  ...
  if (!parsedCity && !parsedCountry && jobLocation) {
    ...
    parsedCountry = parsed.country ? normalizeCountryForMatching(parsed.country) : null;
  }
  ...
  if (jobLocation && (
    ...
    return { matches: true, matchScore: 80, reason: 'Remote/hybrid work available' };
  }
  ...
  for (const targetCity of targetCities) {
    ...
    if (targetCountry && parsedCountry && countryMatches(parsedCountry, targetCountry)) {
      return {
        matches: true,
        matchScore: 70,
...
export function validateLocationCompatibilityEnhanced(
  ...
  if (!userTargetCities || userTargetCities.length === 0) {
    return { compatible: true, matchScore: 100, reasons: ['No location preference'] };
  }
