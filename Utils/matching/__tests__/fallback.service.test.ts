...
const MockScoringService = ScoringService as jest.MockedClass<
	typeof ScoringService
>;
...
describe("FallbackMatchingService", () => {
	...
	beforeEach(() => {
		mockScoringService =
			new MockScoringService() as jest.Mocked<ScoringService>;
	...
	describe("generateRobustFallbackMatches", () => {
		it("should generate fallback matches using scoring service", () => {
			const result = fallbackService.generateRobustFallbackMatches(
				mockJobs,
				mockUser,
			);

			expect(mockScoringService.scoreJobsForUser).toHaveBeenCalledWith(
				mockJobs,
				mockUser,
			);
		...
		it("should respect maxMatches configuration", () => {
			...
			expect(result.length).toBeLessThanOrEqual(
				MATCHING_CONFIG.fallback.maxMatches,
			);
		});
	...
	describe("generateMatchesByCriteria", () => {
		it("should filter by career path when specified", () => {
			const result = fallbackService.generateMatchesByCriteria(
				mockJobs,
				mockUser,
				{
					careerPath: true,
	...
	describe("generateEmergencyFallbackMatches", () => {
		it("should generate emergency matches for recent jobs", () => {
			const result = fallbackService.generateEmergencyFallbackMatches(
				mockJobs,
				mockUser,
			);
		...
		it("should respect maxEmergencyMatches configuration", () => {
			...
			expect(result.length).toBeLessThanOrEqual(
				MATCHING_CONFIG.fallback.maxEmergencyMatches,
			);
		});

		it("should handle jobs without created_at", () => {
			const jobsWithoutDate = mockJobs.map((job) => ({
				...job,
	...
	describe("shouldUseFallback", () => {
		...
		it("should return true when average confidence is below threshold", () => {
			...
			const result = fallbackService.shouldUseFallback(
	...
	describe("getStats", () => {
		it("should return fallback configuration statistics", () => {
			...
			expect(stats.lowConfidenceThreshold).toBe(
				MATCHING_CONFIG.fallback.lowConfidenceThreshold,
			);
			expect(stats.diversityFactor).toBe(
				MATCHING_CONFIG.fallback.diversityFactor,
			);
			expect(stats.emergencyFallbackEnabled).toBe(
				MATCHING_CONFIG.fallback.emergencyFallbackEnabled,
			);
			expect(stats.maxEmergencyMatches).toBe(
				MATCHING_CONFIG.fallback.maxEmergencyMatches,
			);
		});
	});
