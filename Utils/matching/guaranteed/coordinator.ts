...
import { triggerCustomScan, extractMissingCriteria } from "./custom-scan";
...
function combineAndDeduplicate(
  standardMatches: Array<{ job_hash: string; match_score: number; match_reason: string }>,
  guaranteedMatches: Array<{ job: Job; match_score: number; match_reason: string; relaxationLevel?: number }>,
): Array<{ job: Job; match_score: number; match_reason: string; relaxationLevel?: number }> {
  const seen = new Set<string>();
  const combined: Array<{ job: Job; match_score: number; match_reason: string; relaxationLevel?: number }> = [];

  // Create a map of guaranteed matches by job_hash for quick lookup
  const guaranteedMap = new Map<string, { job: Job; match_score: number; match_reason: string; relaxationLevel?: number }>();
...
export async function coordinatePremiumMatching(
  ...
  const locationExpansion = calculateLocationExpansion(
  ...
  if (finalMatches.length >= EXTREME_SUCCESS_THRESHOLD) {
    ...
    const standardMatchesWithJobs = finalMatches
      .map(m => {
  ...
  if (distributionOptions && finalMatches.length > 0) {
    ...
    const jobsWithScores = finalMatches.map(m => {
      const job = allJobs.find(j => j.job_hash === m.job_hash);
      if (!job) return null;
      return {
        ...job,
    ...
    const shouldRelaxDistribution = jobsWithScores.length < distributionOptions.targetCount * 1.5;
    
    distributedJobs = distributeJobsWithDiversity(
      jobsWithScores as any[],
      {
        ...
        ensureWorkEnvironmentBalance: distributionOptions.targetWorkEnvironments.length > 0 && !shouldRelaxDistribution,
    ...
    if (distributedJobs.length === 0 && jobsWithScores.length > 0) {
      distributedJobs = jobsWithScores
        .slice(0, distributionOptions.targetCount)
        .map(j => ({
          job: j,
      ...
      apiLogger.warn("Distribution returned empty, using top matches directly", {
        email: userPrefs.email,
  ...
  if (distributedJobs.length < minMatches) {
    ...
    const missingCriteria = extractMissingCriteria(userPrefs, distributedJobs.map(dj => dj.job));
