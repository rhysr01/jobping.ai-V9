...
export async function fetchActiveUsers(
	...
	if (error) {
		apiLogger.error("Failed to fetch users", error as Error, {
			userCap: limit,
		});
	...
	return users.map((user) => ({
		...
		preferences: {
			...
			work_environment: user.work_environment
				? user.work_environment.toLowerCase().includes("remote")
					? "remote"
					: user.work_environment.toLowerCase().includes("hybrid")
						? "hybrid"
						: user.work_environment.toLowerCase().includes("office") ||
								user.work_environment.toLowerCase().includes("on-site")
							? "on-site"
							: "unclear"
				: undefined,
			...
			visa_sponsorship:
				user.visa_status &&
				!user.visa_status.toLowerCase().includes("eu-citizen") &&
				!user.visa_status.toLowerCase().includes("citizen"),
