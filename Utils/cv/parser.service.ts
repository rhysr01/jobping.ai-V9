...
`;
    ...
    const response = await this.openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a CV parser. Extract structured data and return ONLY valid JSON.'
        },
        {
          role: 'user',
          content: prompt
  ...
  generateWOWInsights(cvData: CVData): string[] {
    ...
    if (cvData.total_years_experience) {
      if (cvData.total_years_experience >= 5) {
        insights.push(`${cvData.total_years_experience} years experience (you're in high demand)`);
      } else if (cvData.total_years_experience >= 2) {
        insights.push(`${cvData.total_years_experience} years experience (perfect for mid-level roles)`);
      }
    }
    ...
    const hasFrontend = cvData.technical_skills.some(s => 
      ['react', 'vue', 'angular', 'frontend'].some(k => s.toLowerCase().includes(k))
    );
    const hasBackend = cvData.technical_skills.some(s => 
      ['node', 'python', 'java', 'backend', 'api'].some(k => s.toLowerCase().includes(k))
    );
