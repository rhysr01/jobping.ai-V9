...
/**
 * @deprecated Use getDatabaseClient() from '../databasePool' directly instead.
 * This function is kept for backwards compatibility with email module exports.
 */
export function getSupabaseClient() {
	// Import the canonical implementation
	const { getDatabaseClient } = require("../core/database-pool");
...
export const assertValidFrom = (from: string): void => {
	...
	if (domain !== "getjobping.com") {
		throw new Error(
			`Invalid sender domain: ${domain}. Expected getjobping.com`,
		);
	}
};
